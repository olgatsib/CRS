...
public class client
{
    ...
    public static void main(String args[]) throws IOException
	{
	    ...
	    if (args.length == 2) {
	    	...
	    	System.out.println ("Usage: java client RMI (or TCP) [rmihost]"); 
			System.exit(1); 
		}
	    	...
	    	try 
	 		{
	 		    ...
	 		    Registry registry = LocateRegistry.getRegistry(server, 2011);
	 		    // get the proxy and the remote reference by rmiregistry lookup
	 		    rm = (ResourceManager) registry.lookup("group11");
	 		    if(rm!=null)
	 			{
	 			    System.out.println("Successful");
	    ...
	    while(true){
			...
			case 2:  //new flight
			    ...
			    try{
					...
					if (tcpMode) {
						if(tcp.addFlight(Id,flightNum,flightSeats,flightPrice))
			...
			case 3:  //new Car
			    ...
			    try {
					...
					if (tcpMode) {
						if(tcp.addCars(Id,location,numCars,price))
			...
			case 4:  //new Room
			    ...
			    try{
					...
					price = obj.getInt(arguments.elementAt(4));
					if (tcpMode) {
						if(tcp.addRooms(Id,location,numRooms,price))
			...
			case 6: //delete Flight
			    ...
			    try{
					...
					flightNum = obj.getInt(arguments.elementAt(2));
					if (tcpMode) {
						if(tcp.deleteFlight(Id,flightNum))
			...
			case 8: //delete Room
			    ...
			    try{
					...
					location = obj.getString(arguments.elementAt(2));
					if (tcpMode) {
						if(tcp.deleteRooms(Id,location))
			...
			case 9: //delete Customer
			    ...
			    try{
					...
					if (tcpMode) {
						if(tcp.deleteCustomer(Id,customer))
			...
			case 10: //querying a flight
			    ...
			    try{
					...
					int seats = 0;
					if (tcpMode) 
						seats=tcp.queryFlight(Id,flightNum);
					else
			...
			case 11: //querying a Car Location
			    ...
			    try{
					...
					location = obj.getString(arguments.elementAt(2));
					if (tcpMode) 
						numCars=tcp.queryCars(Id,location);
					else
			...
			case 12: //querying a Room location
			    ...
			    try{
					...
					location = obj.getString(arguments.elementAt(2));
					if (tcpMode)
						numRooms=tcp.queryRooms(Id,location);
					else
			...
			case 13: //querying Customer Information
			    ...
			    try{
					...
					String bill = "";
					if (tcpMode)
						bill=tcp.queryCustomerInfo(Id,customer);
					else
			...
			case 14: //querying a flight Price
			    ...
			    try{
					...
					flightNum = obj.getInt(arguments.elementAt(2));
					if (tcpMode)
						price=tcp.queryFlightPrice(Id,flightNum);
					else
			...
			case 15: //querying a Car Price
			    ...
			    try{
					...
					location = obj.getString(arguments.elementAt(2));
					if (tcpMode)
						price=tcp.queryCarsPrice(Id,location);
					else
			...
			case 17:  //reserve a flight
			    ...
			    try{
					...
					if (tcpMode) {
						if(tcp.reserveFlight(Id,customer,flightNum))
			...
			case 18:  //reserve a car
			    ...
			    try{
					...
					if (tcpMode) {
						if(tcp.reserveCar(Id,customer,location))
			...
			case 19:  //reserve a room
			    ...
			    try{
					...
					location = obj.getString(arguments.elementAt(3));
					if (tcpMode) {
						if(tcp.reserveRoom(Id,customer,location))
			...
			case 20:  //reserve an Itinerary
			    ...
			    try{
					...
					if (tcpMode) {
						if(tcp.itinerary(Id,customer,flightNumbers,location,Car,Room))
			...
			case 22:  //new Customer given id
			    ...
			    try{
					...
					if (tcpMode)
						customer=tcp.newCustomer(Id,Cid);
					else
						customer=rm.newCustomer(Id,Cid);
			...
			case 23: //Start a transaction
			    ...
			    try{
					if (tcpMode) {
						System.out.println("Does not support TM in TCP mode");
						break;
					}
					else {
						// suppose that MW returns the transaction number,
						// then setting Id in all other methods should be removed,
						// since the ID for transactions will be set at this stage,
						// otherwise I do not see the sense in returning this number.
						Id = rm.startTxn();
			...
			case 26:
				 if(arguments.size()!=2) {
				    ...
				    System.out.print("Crashing a server: ");
				    String which = null;
				    try{
						which = obj.getString(arguments.elementAt(1));
						if (tcpMode) {
							System.out.println("Does not support TM in TCP mode");
							break;
						}
			...
			case 27:
				 if(arguments.size()!=2) {
				    ...
				    System.out.println("Killing a server: ");
				    which = null;
				    try{
						which = obj.getString(arguments.elementAt(1));
						if (tcpMode) {
							...
							System.out.println("Killing after prepeare: " + which);
							rm.setDieAfterPrepare(which);
	...
	// newOlga
		...
		return 24;
	// Do not yet know how it will work
	else if (argument.compareToIgnoreCase("abort") == 0)
		return 25;
	else if (argument.compareToIgnoreCase("crash") == 0)
		return 26;
	else if (argument.compareToIgnoreCase("killafterprepare") == 0)
		return 27;
	else
	    return 666;
